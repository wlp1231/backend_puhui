...
import {addBankExamine, addBankNoExamine, addExamine, addNoExamine, updateExamine} from "@/api/system/examine";


export default {
  name: "bankLoanDetails",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  created() {
    ...
    if (beId) {
      this.loading = true;
      listLoansById(beId).then(response => {
    ...
    if (beId) {
      this.loading = true;
      listMaterialsById(beId).then(response => {
    ...
    if (beId) {
      this.loading = true;
      listAptitudeById(beId).then(response => {
        this.aptitudeList = response.rows;
        this.loading = false;
      });
    }
    this.form.exaLoaId = beId;
  },
  methods: {
    // 下一步
    next() {
...
// 表单重置
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.exaId != null) {
            ...
            addBankExamine(this.form).then(response => {
    ...
    register: function () {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          addBankNoExamine(this.form).then(response => {
            this.msgSuccess("操作成功");
            this.active++;
            //跳转上一层
            this.$router.go(-1);

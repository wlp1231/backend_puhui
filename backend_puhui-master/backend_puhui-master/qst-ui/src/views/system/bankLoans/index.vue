...
<!--      <el-table-column label="到期时间" align="center" prop="loaEndTime" width="100">
        ...
        <template slot-scope="scope">
          ...
          <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)"
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="handleLoanDetails" icon="el-icon-circle-check"
                                v-hasPermi="['system:loans:loan']" :disabled="scope.row.loaState=='4'">进入审批
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="企业编号" prop="loaComNo">
        ...
        <el-form-item label="银行" prop="loaBankId">
          <el-select v-model="form.loaBankId" filterable placeholder="请选择银行">
            <el-option
              v-for="dict in bankNames"
              :key="dict.bankId"
              :label="dict.bankName"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBankLoans(this.queryParams).then(response => {
    ...
    /** 查看详情操作 */
    handleLoanDetails: function (row) {
